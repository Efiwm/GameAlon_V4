...
namespace GameAlon
{
    class Program
    {
            ...
            MainMenu.Keyboard = new KeyboardButton[][]
                {
                    ...
                    new KeyboardButton[]
                    {
                        new KeyboardButton("👥برترین کاربران👥")
                        ,
                    ...
                    new KeyboardButton[]
                    {
                        ...
                        ,
                        new KeyboardButton("📋درباره ی ربات📋")
                    }
                };
            ...
            while (true)
            {
                ...
                try
                {
                    foreach (Update Update in Updates)
                    {
                        ...
                        {
                            SendMessage Message01 = new SendMessage(Update.Message.Chat.Id, "ربات درحال حدس عدد میباشد.") { ReplyMarkup = MainMenu };
                            await bot.MakeRequestAsync(Message01);
                            foreach (Person item in UserList)
                            {
                                ...
                                {
                                    SendMessage Message02 = new SendMessage(Update.Message.Chat.Id, "ربات را دوباره استارت کنید") { ReplyMarkup = MainMenu };
                                ...
                                foreach (Update UserGuessing in Updates)
                                {
                                    ...
                                    {
                                        item.Score = +1;
                                        SendMessage Message02 = new SendMessage(Update.Message.Chat.Id, "عدد را درست حدس زدید.") { ReplyMarkup = MainMenu };
                                        await bot.MakeRequestAsync(Message02);
                                    }
                                    else
                                    {
        ...
        public static bool Exist(long ID, List<Person> UserList)
        {
            bool Exist = false;
            //Employee E1 = new Employee(Useraname , Password);
            foreach (Person item in UserList)
            {
                if (ID == item.ID)
                {
        ...
        //public static bool Exist(List<Person> UserList)
        //{
        //    //    bool Exist = false;
        //    //    //Employee E1 = new Employee(Useraname , Password);
        //    //    foreach (Person item in UserList)
        //    //    {
        //    //        if (item.UserPlayedGuessGame = false)
        //    //        {
        //    //            Exist = true;
        //    //        }
        //    //        else
